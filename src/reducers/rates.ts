...
export const rates = (state:IRatesState = initialState, action):IRatesState=> {
  switch (action.type) {
    case SET_RATES : {
...
const getCurrencyRate = (code:string) => (state:IRatesState)=>{ //возможно вынести все коды в enum
  return state.rates.find(currency=>currency.code === code).rate
}
/** к валютам в массиве добавляет их курсы **/
export const getRatesArray = (defaultArray:IDefaultCurrency[]) => (state:IRatesState)=> {
  return defaultArray.map(currency => {
...
const createList = (array, eur,rub,usd,cny):Ilist => {
...
const sortByUSD = (array)=>{
  return [...array].sort((a,b)=>{
    return a.toUSD - b.toUSD
  })
}
const sortByEUR = (array)=>{
  return [...array].sort((a,b)=>{
    return a.toEUR - b.toEUR
  })
}
const sortByRUB = (array)=>{
  return [...array].sort((a,b)=>{
    return a.toRUB - b.toRUB
  })
}
const sortByCNY = (array)=>{
  return [...array].sort((a,b)=>{
    return a.toCNY - b.toCNY
  })
}
